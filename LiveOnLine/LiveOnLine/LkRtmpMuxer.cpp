...
LkRtmpMuxer::LkRtmpMuxer()
{
}

int LkRtmpMuxer::addH264(const LkStream &data, unsigned long long dts)
{
    return m_rtmpAU->sendPacket(data, dts, RTMP_PACKET_TYPE_VIDEO, StreamChannel_Video);
}

int LkRtmpMuxer::addAAC(const LkStream &data, unsigned long long dts)
{
    return m_rtmpAU->sendPacket(data, dts, RTMP_PACKET_TYPE_AUDIO, StreamChannel_Audio);
}

int LkRtmpMuxer::setMetaData(const FlvMetaData &metaData)
{
    m_metaData.clear();
    ...
    p = put_byte(p, AMF_STRING);
    p = put_amf_string(p, LK_NAME);

    p = put_amf_string(p, "authors");
    p = put_byte(p, AMF_STRING);
    p = put_amf_string(p, LK_AUTHORS);
...
int LkRtmpMuxer::setMetaData(const FlvMetaData &metaData, LkStream &body_data)
{
	m_metaData.clear();
...
void LkRtmpMuxer::setRtmpUrl(const string &url)
{
    m_rtmpUrl = url;
}

int LkRtmpMuxer::start()
{
...
int LkRtmpMuxer::stop()
{
    return m_rtmpAU->close();
}

string LkRtmpMuxer::genSequenceHeader()
{
    string seq;
...
string LkRtmpMuxer::genVideoFrame(char *data, int size, unsigned long long pts, unsigned long long dts, bool keyframe)
{
    string frame;
...
string LkRtmpMuxer::genAudioFrame(char *data, int size, bool sequence)
{
    string frame;
    frame.append(1, 0xAF);
    if (sequence) {
...
int LkRtmpMuxer::addH264Internal(char *data, int size, unsigned long long pts, unsigned long long dts)
{
    string pkt;
...
int LkRtmpMuxer::addAACInternal(char *data, int size, unsigned long long pts)
{
